...
//     try {
//       await axios.post('/api/suppliers/history.json', {
//         transactionId: supplierData.id,
//         amount,
//       });
//       setAmount('');
...
//                   <button onClick={() => setShowConfirm(false)}  type="button" className="btn btn-outline border border-gray-200 w-100 rounded f-size-12 fw-medium d-flex align-items-center justify-content-center"><i className="icon ico-edit margin-r-10"></i> Edit</button>
//                 </div>
//               )}
//             </div>
//           </div>
...
import ConfirmPaymentResult from '../../components/Dialogs/ConfirmPaymentResult/ConfirmPaymentResult';
...
const handlePayNow = async () => {
  try {
    const newTransaction = {
      ...
      receiverName: supplierData.name,
      thriveId: supplierData.thriveId,
      date: new Date().toISOString(),
      amount: parseFloat(amount),
    };

    console.log("Payment saved locally:", newTransaction);
          ...
          <div className="tab-pane fade show active">
            <div className="w-100 mw-sm-340 view-supplier-contant view-supplier-pay-contant center-block">
              <div className="bg-primary border-radius-14 w-100 balance-box rounder-sm-0 padding-30 margin-b-20 text-center">
                <h5 className="text-white fw-medium mb-2">Balance</h5>
                <div className="f-size-10 text-gray-400 fw-semibold text-uppercase mb-2">Available Amount</div>
                <h2 className="text-white fw-medium mb-2">R{balance ? balance.toFixed(2) : '0.00'}</h2>
              </div>
              ...
              {paymentResult && (
                <ConfirmPaymentResult result={paymentResult} onClose={() => setPaymentResult(null)} amount={amount} supplierName={supplierData.name} />
              )}
            </div>
          </div>
        )}
